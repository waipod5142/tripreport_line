...
import { deleteImages } from "../services/cloudinaryUpload";
...
export async function deleteTrip(req: Request, res: Response) {
  try {
    ...
    if (deleted.imageUrl) {
      // best-effort: the trip row is already gone, so a failed cleanup only leaves an orphaned asset
      await deleteImages([deleted.imageUrl]).catch((err) =>
        console.error("Cloudinary cleanup failed for deleted trip:", err)
      );
    }
